        ...
        <div class="d-flex align-center">
          ...
          <v-tooltip location="bottom">
            ...
            <div style="max-width: 260px">
              <strong>% of Project Revenue</strong>
              <br />
              Employee expense divided by total project revenue.

              <br /><br />

              <strong>% of Project Expense</strong>
              <br />
              Employee expense divided by total project expense.
            </div>
          </v-tooltip>
        </div>
      </div>
...
const selectedEmployees = computed({
  ...
  set: (value) => {
    appStore.saveEmployeePreferences(value);
  },
});
...
const chartData = computed(() => {
  const filteredEmployees = appStore.employeeData
    .filter((emp) => {
      if (selectedEmployees.value.length === 0) return true;
      return selectedEmployees.value.includes(emp.name);
    })
  ...
  const revenueData = filteredEmployees.map((emp) =>
    Math.round(emp.percProjectRevenue),
  );
  const expenseData = filteredEmployees.map((emp) =>
    Math.round(emp.percProjectExpense),
  );
